		...
		public :
			...
			void generateValuesFromBuffer(LArray<Primitives::Float32> vertices) override 
			{
				...
				for (unsigned int i = 0; i < vertices.getSize()/3; i++) {
					...
					if (vertices.getElement(3 * i + 1) > max_y) max_y = vertices.getElement(3 * i + 1);
					if (vertices.getElement(3 * i + 1) < min_y) min_y = vertices.getElement(3 * i + 1);
					if (vertices.getElement(3 * i + 2) > max_z) max_z = vertices.getElement(3 * i + 2);
					if (vertices.getElement(3 * i + 2) < min_z) min_z = vertices.getElement(3 * i + 2);
				}
				min.x = min_x;
				min.y = min_y;
				min.z = min_z;
